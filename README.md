...
- **실측**: 실패 시나리오 **12건 전부 착수(10건 완료)**, 계측 **7장 중 5장**. 전부 운영 중인 t3.small 에서 잰 값입니다
...
> **범위 안 12건 전부 착수 · 10건 완료 + 2건 부분** → [docs/failure-scenarios.md](docs/failure-scenarios.md)
...
| | 무엇 | 왜 아직인가 |
|---|---|---|
| **지표** | **반영률 1.0% 의 원인** | 되읽기를 붙였더니 `204` 를 받은 **508건 중 5건만** 매체 보고서에 있었습니다. 처리 지연인지 버스트 스로틀링인지 가르는 실험(20건 천천히 / 20건 몰아서)을 걸어 뒀습니다 |
| D-3 | 결제 웹훅 중복·순서 역전 | `payments` 가 마이그레이션에만 있습니다. **범위 축소가 아니라 아직 안 만든 것**입니다 |
