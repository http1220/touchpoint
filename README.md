...
- **상태**: **파이프라인 관통** — 광고 클릭 → 방문·접점 적재 → 수집(CORS) → 전환 → 아웃박스 → 워커 → **GA4 도달 확인** (2026-09-12). GA4 로 **543건 실전송**, Data API 로 되읽어 대조 → **도달률 100% · 반영률 95.2%** (2026-09-14)
- **실측**: 실패 시나리오 **12건 전부 착수(10건 완료)**, 계측 **7장 중 5장**. 전부 운영 중인 t3.small 에서 잰 값입니다
- **읽을 것이 하나라면**: [예상이 틀린 곳 넷](#예상이-틀린-곳-넷--이-저장소에서-제일-볼-만한-부분) — `204` 를 받은 전송의 5%가 매체에 없던 이유, 301이 위험한 진짜 이유, `SKIP LOCKED` 가 처리량을 안 벌어 준 이유, 오픈 리다이렉트의 과녁이 `Host` 였던 이유
...
| **매체 도달률** (`2xx` 응답) | ≥ 99% | GA4 **543 / 543** | ✅ |
| **매체 반영률** (보고서에 존재) | ≥ 99% | GA4 **517 / 543 = 95.2%** | **✗ 목표 미달** |

> **지표 하나가 사실 둘이었습니다.** `204` 를 받은 543건을 GA4 Data API 로 되읽어 한 건씩 맞대 보니, 매체 보고서에 남은 것은 **517건(95.2%)** 이었습니다. 같은 전송을 두고 **도달률 100% 와 반영률 95.2% 가 동시에 참**입니다 — 이름 하나로 뭉쳐 두면 둘 중 편한 쪽이 화면에 뜹니다.
>
> 보내자마자 물었을 때는 **1.0%** 였습니다. 그 숫자를 믿었으면 "매체가 99%를 버린다"를 문서에 박을 뻔했는데, 처리 지연이었고 7시간 뒤 93.6%가 됐습니다. 꼬리는 **25시간**까지 끌리다 멈췄고, **잔여 약 5%는 영구 유실**입니다. 전송 속도와는 무관합니다 — 가장 빠르게 보낸 무리의 누락률이 오히려 가장 낮았습니다(3.2% vs 6.4%).
>
...
| "`204` 를 받았으니 전송은 성공" | **같은 500건의 반영률이 95.2%** 였습니다. 게다가 보내자마자 재면 **1.0%** 로 나와서, 그 숫자를 믿었으면 "매체가 99%를 버린다"를 박을 뻔했습니다 |
...
| | 무엇 | 왜 아직인가 |
|---|---|---|
| 지표 | **잔여 5% 의 원인** | 되읽기로 **반영률 95.2%** 까지 확정했습니다. 남은 5%가 왜 사라지는지는 매체 내부라 밖에서 규명할 수 없습니다 — 규모를 아는 것까지가 우리 몫입니다 |
