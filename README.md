...
지금 실제로 돌아가고 있습니다 → **[시연 안내](https://sshwan.com/tour)**(여기부터) · [웹툰 홈](https://sshwan.com/) · [광고 랜딩](https://lp.sshwan.com/l/1) · [지표 화면](https://app.sshwan.com/metrics)
...
| | |
|---|---|
| 실제 PG 연동 | **이니시스 테스트 상점(카드)만** 붙어 있고, [시연 안내](https://sshwan.com/tour#pay)의 버튼으로 누구나 해 볼 수 있습니다(카드 승인은 실제로 일어나고 자정 전 자동 취소). 운영에서 확인한 것은 결제창이 우리 서명을 받아 열리는 것, 결제창을 닫는 것, 위조된 승인 주소를 거절하는 것, 환불 API 인증까지입니다. **실카드 승인 · 망취소 · 환불 성공 경로는 운영에서 확인하지 않았습니다**(단위 테스트로만). 페이팔은 아직입니다 → [계획](docs/plan-multi-pg.md) |
