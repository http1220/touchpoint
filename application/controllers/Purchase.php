 ...
 * 없다 → 계획 12장 결정 5
 *
 * **09-19 오후부터 이 방어선에 문이 하나 있다** — 카드 없는 시연 버튼
 * (`/pay/stub/confirm`)이 서버 대신 서명한다. 받는 결제는 "그 버튼이 방금 만든
 * demo- 결제" 뿐이라 기존 결제는 못 건드리지만, **누구나 새 시연 결제를 captured
 * 로 만들 수 있고 그 전환은 매체로 간다**(사용자 결정) → docs/plan-multi-pg.md 6장
 *
