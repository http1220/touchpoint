...
if ( ! function_exists('m_rate')):
	/**
	 * **비율은 분모와 함께만 낸다.** "95.2%" 가 아니라 "517 / 543 (95.2%)".
	 *
...
<main>
  ...
  <div class="note">
    ...
    페이로드가 틀려도 <code>204</code> 를 준다 — 도달률 100% 와 반영률 95.2% 가 같은 전송을 두고 동시에 참이었다.<br>
    반영률은 <strong>매체에서 되읽어 한 건씩 맞대야</strong> 나오고, 그 결과를 남기는 자리가 지금 스키마에 없다.
    그래서 화면이 지어내지 않고 명령만 적는다 —
    <code>php public/index.php cli/verify ga4 2026-09-13 today</code><br>
    문서에 남은 실측값: GA4 <strong>517 / 543 (95.2%)</strong>, 전송 +25시간 시점
    (<code>docs/benchmarks.md</code> 5-1). <span class="none">이 화면이 계산한 값이 아니다.</span>
    되읽기는 매체의 처리 지연을 타므로 <strong>전송 직후에 물으면 1%</strong> 가 나온다 — 다음 날 돌린다.<br>
