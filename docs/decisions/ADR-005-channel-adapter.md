...
| **픽셀 — 방침과 거부 수단을 먼저 만들고 켰다** (09-15, `META_PIXEL_BROWSER=true`) | `_fbp` 가 생겨 매칭이 강해진다. 켜기 전에 `/privacy`(행태정보·국외이전 고지)와 `/privacy/ads`(거부 쿠키 → 픽셀 미출력 · 서버 전환에 맥락 미탑재 · 기존 `_fbp`·`_fbc` 삭제)를 먼저 올렸다. PageView 와 쿠키 발급만 하고 **전환은 서버만 보낸다** — 결제 확정은 웹훅이라 브라우저가 그 시점의 `event_id` 를 모르고, 합칠 키 없는 이중 전송이 된다 |
