...
그래서 어댑터의 사전 거절(A)을 **풀지 않았다.** 풀 근거가 "API 가 받아 줬다" 뿐이기 때문이다. B1~B3 가 Events Manager 에서 어떻게 보이는지(테스트 이벤트 탭에 나타나는가, 진단에 경고가 뜨는가)를 확인한 뒤 판정한다. Meta 는 확인까지 20분을 두라고 적어 두었다 — GA4 반영률을 +25h 에 판정했다가 틀린 일([benchmarks 5-1](../benchmarks.md))이 있어 이번에는 처리 창을 다 기다린다.
