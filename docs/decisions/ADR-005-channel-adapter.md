...
| **픽셀은 이번에 넣지 않는다** | 넣으면 `_fbp` 가 생겨 매칭이 강해지지만, 쿠키 고지·처리방침(행태정보·국외이전) 문구가 함께 필요하다. 따로 결정한다 |

**`/conversion` 경로는 아직 싣지 않는다.** 크로스오리진이라 `Referer` 에 경로가 없고(위 표), 본문으로 URL 을 받는 계약 변경이 따로 필요하다.
...
> **워커로 보내지 않고 어댑터를 직접 불렀다.** 워커 컨테이너가 `profiles: ["worker"]` 뒤에 있어 09-12 22:49 이후 떠 있지 않았고, 대기 행이 9건(`noop` 3 · `ga4` 6, 전부 실험 트래픽) 쌓여 있었다. 워커를 켜면 그 9건이 운영 GA4 로 나가거나 `어댑터 없음` 으로 dead 가 된다. 어댑터와 payload 는 워커가 쓰는 것과 같다.
