...
### 정정 (2026-09-12) — 원문을 확보했다

09-08에는 상세요강이 이미지라 스킬 태그와 일부 문장만 보고 판단했다. 09-12에 세 공고의 상세요강 원문을 전부 확보해 대조했다. 바뀐 것은 둘이다.

- **셋 다 지원한다.** 콘텐츠 플랫폼 공고를 "지원 안 함"으로 뒀는데, 그 공고의 우대사항이 `PHP·CodeIgniter` / `레거시 구조 최적화` / `다국어` / `기술 부채 개선`이다. **이 프로젝트가 가장 직접적으로 대응하는 항목들**이다
- **필수와 우대의 경계가 분명해졌다.** 아래 표의 "필수" 칸은 셋 다 도메인 1개로 충족된다

| # | 공고 | 필수의 색 | 우대의 색 | 이 저장소에서 앞세울 것 |
|---|---|---|---|---|
| **1** | 플랫폼 개발 및 **광고 매체 연동** | HTTP·Cookie·URL Parameter·MySQL·Git | GA·GTM / Meta Pixel·Google Ads / 외부 API·SDK / **Domain·Redirect·CORS** / PHP·CI | 어트리뷰션 파이프라인, 쿠키·파라미터·리다이렉트, CORS, GA4 전송 |
| **2** | **서비스&인프라 운영** | Linux·웹서버 요청 흐름·MySQL/PostgreSQL | 서버 직접 구성 / **Nginx** / **Docker·K8s** / Prometheus·Grafana·Jenkins·Airflow | EC2 직접 구축, OpenResty 설정, 복제, 구조화 로그, **배포하며 잡은 장애 6건** |
| **3** | **콘텐츠 플랫폼** 개발 | PHP 개발 경험·MySQL·Git | **PHP·CodeIgniter** / Linux·Docker / **다국어·대규모 B2C** / **기술 부채 개선** | CI3 레거시 위 구조 분리([ADR-017](decisions/ADR-017-ci3-application-structure.md)), 다국어 설계, ADR·worklog 자체 |

#1 공고의 업무 문장 하나가 이렇다.

> "**도메인·Redirect 등 다양한 유입 환경에서의 사용자 및 전환 추적 기능 개발**"

그리고 같은 공고의 우대사항에 이 줄이 있다.

> "**Domain, Redirect, CORS** 등 다양한 웹 환경에서 발생하는 **데이터 전달 이슈**를 다뤄보신 분"
...
방향(애드테크냐 · 인프라냐 · 콘텐츠 플랫폼이냐)은 아직 정하지 않았다. 그래도 "왜 이 포지션인가"는 세 면접 어디서나 나오므로 **공통 축**을 하나 잡는다.

> **"안 보이는 것을 숫자로 만드는 일을 해왔습니다."**
> 어트리뷰션은 유입이 어디서 새는지를 재고, 모니터링은 시스템이 어디서 느려지는지를 재고,
> 레거시 개선은 기술 부채가 어디서 비용을 만드는지를 잰다.
> 셋 다 측정이고, 기존 경력(응답을 7단계로 쪼개 병목 특정, 인식률 68%→92.6%)도 측정이다.
...
| **M4** 수집·전송 | 09-16~19 | **노출/클릭 커스텀 수집 동작** (09-15 완료 — `/impression` 배치 · `/click` 302 · 자리별 CTR) · 아웃박스 → GA4 전송 · 워커 4개에서 중복 0 |
...
| 4 | ~~Meta CAPI~~ | 어댑터(09-14, 실측 6파일) · 결제 경로 브라우저 맥락 보존(09-15) · `/conversion` 경로·픽셀 코드(09-15). 운영 전송 전환(09-15). 개인정보처리방침·거부 수단 후 픽셀 켬(09-15). 남은 것은 실전환이 쌓인 뒤 EMQ 확인 → [ADR-005](decisions/ADR-005-channel-adapter.md) |
