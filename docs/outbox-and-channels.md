...
**① 보내자마자 물으면 1.0% 다.** 그 숫자를 믿고 "매체가 99%를 버린다" 로
적을 뻔했다. 처리 지연이었고 7시간 뒤 93.6%가 됐다.
**되읽기 배치는 다음 날 돌린다** — 이 문서가 앞에서 "지연" 을 함정으로
적어 둔 이유가 숫자로 확인됐다.

**② 꼬리는 25시간까지 끌리고 거기서 멈춘다.** 유입 속도가
1.10 → 0.75 → 0.10건/h 으로 0에 수렴했다. **잔여 약 5%는 영구 유실**이다.

**③ 속도를 낮춰도 소용없다.** 전송 속도와 누락률에 상관이 없었다 —
오히려 가장 빠르게 보낸 무리(36건/초)의 누락률이 가장 낮았다(3.2% vs 6.4%).
스로틀링이 아니므로 **천천히 보내는 것은 대책이 아니다.**

원인은 매체 내부라 밖에서는 규명할 수 없다. 하지만 **규모를 아는 것만으로
값을 한다** — 매체 대시보드와 자체 집계가 5% 어긋날 때
**우리 쪽 버그부터 뒤지지 않게** 되기 때문이다.
