...
`204` 를 받은 **543건**을 되읽어 맞댔다. 전송 로그상으로는 100% 성공이다.

| 전송 후 경과 | 반영률 |
|---|---|
| +0h (보내자마자) | **1.2%** — 6 / 508 ¹ |
| +7h | 93.6% |
| +25h | 95.2% ← 여기서 "수렴·영구 유실 4.8%" 로 **잘못** 판정 |
| **+40h** | **100.0%** ← 전부 들어왔다 |

> ¹ 이 줄만 분모가 508 이다 — 그때까지 보낸 것이 그만큼이었다. 나머지 줄의 분모는 543.
>   전에는 이 줄도 1.0% 로 적혀 있었는데 어느 분모로도 나오지 않는 값이었다 → benchmarks.md 5-1 각주 (2026-09-18).

**세 가지를 배웠다.**

**① 보내자마자 물으면 1.2% 다.** 처리 지연이었다. **되읽기 배치는 매체의 처리
창을 다 기다린 뒤에 돌린다** — GA4 는 최대 48시간을 적고, 실측은 40시간이었다.

**② 감속은 수렴이 아니다.** +25h 에 유입이 0.10건/h 로 떨어진 것을 보고
"잔여 4.8%는 영구 유실" 이라고 **이 문서와 README 에 적었다.** 사용자가 GA4
보고서를 내려받아(`Purchasers 544`) 보여 준 뒤 다시 대조하니 +40h 에 **543/543** 이었다.
**느려진 것을 멈춘 것으로 읽었다.** ①과 같은 실수를 조금 늦게 한 번 더 한 것이다.

**③ 진짜 영구 누락은 우리가 만든 5건뿐이었다.** 아래 "함정 셋" 의
검증 엔드포인트 전송분 — 매체 잘못이 아니라 **적재를 요청하지 않은 것**이다.

수치와 무리별 분해 → [benchmarks 5-1](benchmarks.md)
...
### 함정 셋 — `sent` 가 곧 "적재 요청" 은 아니다

되읽기를 처음 돌렸을 때 누락 40건 중 **5건은 매체 잘못이 아니었다.**
`GA4_DEBUG=true` 로 나간 것들인데, `/debug/mp/collect` 는 페이로드를
**검사만 하고 적재하지 않는다.** 응답이 성공이라 아웃박스는 `sent` 로 적고,
대조하면 영원히 `missing` 이다.

```
http_status 204  →  운영 엔드포인트. 적재된다
http_status 200  →  검증 엔드포인트. 적재되지 않는다
```

`cli/verify` 는 `204` 만 분모에 넣는다. 상태 코드로 가르는 것이 암묵적이라,
**"어느 엔드포인트로 보냈는가" 를 행에 남기는 편이 낫다** — 지금 스키마에는
그 자리가 없다.

> `sent` 라는 상태 이름이 **"보냈다"** 까지만 뜻한다는 것을 여기서 다시 만난다.
> 보낸 것과 받아들여진 것과 집계된 것은 셋 다 다르다.
