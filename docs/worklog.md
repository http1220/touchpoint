...
- [ ] **C 장 — 리다이렉트** (C-1 301 vs 302 · C-2 다단 리다이렉트 파라미터 유실). `.env` 스위치가 이미 있어 빠르고, **공고 셋 모두에 `Redirect` 가 걸려 있다**
- [ ] 커스텀 수집 `/impression` · `/click`
- [ ] D-2(매체 타임아웃) · E-1(복제 지연) — 실패 시나리오 10개 중 현재 3개
- [ ] 09-19 지원 전: README · 아키텍처 다이어그램 · 실패 문서 마감
- [ ] (선택) 추적 도메인을 사면 A-1 · A-2 가 열린다. 코드는 이미 준비돼 있다
