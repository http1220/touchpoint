...
**운영 cron 추가** 매일 05:13 UTC(14:13 KST, 파기 배치 30분 뒤) `cli/verify payments 90` → `~/logs/verify-payments.log`. 수동 1회 실행으로 로그 형식 확인(시각 · 결과 · `exit=1`). 대조군 1건은 12월 중순 90일 창을 벗어날 때까지 매일 `exit=1` 로 남는다.
