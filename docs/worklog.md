...
- [ ] A-3 same-site · cross-origin 대조 · D-3 중복 전환 — 범위 안 남은 둘
- [ ] **실제 매체 표본.** 지금 전송 로그의 99.9%가 `NoopChannel` 이라
      성공률이 파이프라인이 아니라 가짜 채널의 성실함을 재고 있다
- [ ] 파기 배치 cron 등록 (`plan` 을 며칠 지켜본 뒤)
- [ ] 커스텀 수집 `/impression` · `/click`
- [ ] 저장소 Topics (값은 09-12 항목에 적어 뒀다)
