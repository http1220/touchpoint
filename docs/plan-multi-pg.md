...
| 닫기 페이지 | 결제창(다른 오리진) 안 프레임에서 열리면 엣지의 `X-Frame-Options: SAMEORIGIN` 에 막힐 수 있다 [추정] → **09-19 확인: 닫기 URL 이 불리고(200) 결제창은 닫힌다.** 우리 페이지가 프레임 안에서 그려졌는지는 가려내지 못했다 — 보기에 문제는 없어 두었다 |
